...
export function UploadPdfDocumentForm() {
  ...
  const ingest = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const response = await fetch("/api/retrieval/file_upload/pdf", {
      method: "POST",
      body: formData,
    });
  ...
  return (
    <form onSubmit={ingest} className="flex w-full mb-4">
      <input type={"file"} id={"file-upload"} onChange={handleFileUpload} className="w-full" />
      <button
        type="submit"
