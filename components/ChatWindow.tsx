...
export function ChatWindow(props: {
  ...
  const { messages, input, setInput, handleInputChange, handleSubmit, isLoading, setMessages } =
    useChat({
      api: endpoint,
    });
  ...
  async function sendMessage(e: FormEvent<HTMLFormElement>) {
    ...
    // Some extra work to show intermediate steps properly
      ...
      setInput("");
      const newMessages = messages.concat({ id: messages.length.toString(), content: input, role: "user" });
      ...
      if (response.status === 200) {
        ...
        const intermediateStepMessages = (json.intermediate_steps ?? []).map((intermediateStep: AgentStep, i: number) => {
          return {id: (newMessages.length + i).toString(), content: JSON.stringify({
            action: {log: intermediateStep.action.log},
            observation: intermediateStep.observation,
          }, null, 2), role: "system"};
        });
        setMessages([...newMessages, ...intermediateStepMessages, { id: (newMessages.length + intermediateStepMessages.length).toString(), content: json.output, role: "assistant" }]);
      } else {
        if (json.error) {
  ...
  return (
    <div className={`flex flex-col items-center p-8 rounded grow max-h-[calc(100%-2rem)] ${(messages.length > 0 ? "border" : "")}`}>
      ...
      <form onSubmit={sendMessage} className="flex w-full flex-col">
        ...
        <div className="flex w-full mt-4">
          ...
          <button type="submit" className="shrink-0 px-8 py-4 bg-sky-600 rounded w-28">
            <div role="status" className={`${(isLoading ?? intermediateStepsLoading) ? "" : "hidden"} flex justify-center`}>
            ...
            <span className={(isLoading ?? intermediateStepsLoading) ? "hidden" : ""}>Send</span>
          </button>
        </div>
      </form>
    </div>
